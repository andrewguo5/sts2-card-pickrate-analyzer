...
Usage:
    mbgg-sts2-upload --server https://your-server.com --access-code YOUR_SECRET_CODE
    mbgg-sts2-upload  # Uses defaults: localhost and prompts for access code
"""
...
def check_missing_hashes(server: str, access_code: str, hashes: List[str]) -> List[str]:
    ...
    try:
        ...
        if e.response.status_code == 403:
            ...
            print(f"✗ Error checking hashes: HTTP {e.response.status_code}")
            sys.exit(1)
...
Examples:
  ...
  mbgg-sts2-upload --server https://mbgg-api.up.railway.app --access-code mycode123
  ...
  mbgg-sts2-upload --dry-run
    ...
    if uploaded > 0:
        print("\n✓ Upload successful!")
        print(f"  View stats at: {args.server.replace('api.', '').replace(':8001', '')}")
